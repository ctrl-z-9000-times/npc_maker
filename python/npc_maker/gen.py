...
The tools in this module assume that the genome has a specific data format.
In addition to being JSON-encodable objects, these tools expect the genome to be
an array of chromosomes, where each chromosome is a dictionary with the entry:
    {"name": global-innovation-number}.
...
def _genome_select(genome, num_parents=0):
    ...
    to a dictionary of chromosomes, by randomly selecting one chromosome
    from each list.
...
def monoploid_crossover(parents) -> 'genome':
    ...
    child = _genome_select(child, num_parents)
    # Transform the genome back into an array.
...
def polyploid_crossover(parents) -> 'genome':
    ...
    for genome in parents:
        ...
        _genome_select(gamete, num_parents)
        child.extend(gamete)
    return child
