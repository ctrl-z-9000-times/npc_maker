...
from .evo import Individual, Service
...
def _load_env_spec(env_spec_path):
    ...
    if not env_path.is_absolute():
        env_path = env_spec_path.parent.joinpath(env_path).resolve()
...
class Environment:
    ...
    def __init__(self, services, env_spec, mode='graphical', settings={},
        ...
        Argument services is a dict of evolution Service's, indexed by population name.
                 Every population must have a corresponding Service instance.
        ...
        assert all(isinstance(evo, Service) for evo in self.services.values())
    ...
    def poll(self):
        ...
        while True:
            ...
            if "New" in message:
                ...
                evo     = self.services[pop]
                ctrl    = evo.controller()
                genome, info = evo.birth([])
                child   = Individual(genome,
                        environment=env, population=pop, controller=ctrl, info=info)
                self._birth(child)
                ...
                evo     = self.services[pop]
                ctrl    = evo.controller()
                genome, info = evo.birth([p.get_name() for p in parents])
                child   = Individual(genome,
                        environment=env, population=pop, controller=ctrl, info=info,
                        parents=len(parents))
                for p in parents:
...
def score(name, score):
    """
    Report an individual's score or reproductive fitness to the evolution service.
...
def death(name):
    """
    Notify the evolution service that the given individual has died.
