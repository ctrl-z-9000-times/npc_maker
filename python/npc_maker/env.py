...
def _load_env_spec(env_spec_path):
    ...
    for pop in env_spec["populations"]:
        ...
        for interface in pop["interfaces"]:
            ...
            assert isinstance(interface["gin"], int) # TODO: Attempt to type-cast instead of just asserting.
        ...
        assert len(interface_names) == len(set(interface_names)), "duplicate interface names in population specification"
    ...
    def poll(self):
        ...
        while True:
            ...
            if "New" in message:
                ...
                ctrl    = evo.controller()
                genome  = evo.birth([])
                child   = Individual(env, pop, ctrl, genome)
                self._birth(child)
                ...
                self.evolution[population].death(individual)
