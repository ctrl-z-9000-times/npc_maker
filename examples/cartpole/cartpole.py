...
class CartpoleEnvironment:
    def __init__(self):
        spec, mode, settings = env_api.get_args()
        self.spec = spec
        self.mode = mode
    ...
    def birth(self, individual, genotype, **extra):
        self.reset_individual()
        self.individual = individual # UUID name string
        self.ctrl       = Controller(json.dumps(genotype))
    ...
    def advance_controller(self, position, velocity, angle_1, angle_2, wind):
        ...
        wind_gin     = 4
        motor_gin    = 5
        self.ctrl.input(bias_gin,     1.0)
        self.ctrl.input(angle_1_gin,  angle_1)
        self.ctrl.input(position_gin, position)
        self.ctrl.input(wind_gin,     wind)
        if self.num_poles >= 2:
            self.ctrl.input(angle_2_gin, angle_2)
        # 
        self.ctrl.advance(self.dt)
        # Write the motor outputs into the mujoco simulation.
        output = self.ctrl.output(motor_gin)
    ...
    def report_death(self):
        ...
        if self.num_trials:
            ...
            env_api.info(
                individual = self.individual,
                info = {"generalization": successful_trials / self.num_trials})
        env_api.death(individual=self.individual)
        self.reset_individual()
    ...
    def poll_requests(self):
        """ Process all communications from the NPC Maker. """
        while request := env_api.poll():
            ...
            # Save & Load are unsupported, ignore them.
            elif request == "Save":
                pass
            elif request == "Load":
                pass
                ...
                env_api.eprint('Unrecognized Request:', request)
