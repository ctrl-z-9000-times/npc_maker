...
| `"path"`   | String or List of Strings | Required | Filesystem path of the environment's executable program, relative to this file |
...
The "**interfaces**" attribute describes the connection between an agent's body
...
environmental parameters which will be presented in the graphical user
interface in a settings menu for this environment. The user is free to modify
these settings, but only before starting the environment program. The following
table shows all of the expected attributes of settings objects, and extra
...
specification. The settings may be in any order. Missing settings are filled in
with default values. Unexpected settings are rejected. The NPC Maker API
parses, cleans, and presents the settings to the environment program in a
dictionary.
...
received. These messages should not be acknowledged. The following table shows
all of the messages related to managing individuals.

| Message Type | Sender | Receiver | Description |
| :----------- | :----: | :------: | :---------- |
| New   | Environment | Management | Request a new individual from the evolutionary algorithm |
| Mate  | Environment | Management | Request to mate individuals together. This requires at least one parent. This accepts more than two parents. All parents must be alive, in this environment, and of the same population |
...
| Info  | Environment | Management | Associate some extra information with a living individual. The data is kept alongside the individual in perpetuity and may be displayed to the user. The info is stored in the management process |
...
| `"parents"`     | List of Strings | The UUIDs of the parents. Empty if created by a "New" request |
| `"population"`  | String | The name of this individual's population |
| `"controller"`  | List of Strings | The controller program's command line invocation |
| `"genome"`   | Anything | The genetic parameters for the new controller |
