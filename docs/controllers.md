...
This document describes the interface between environments and the control
...
program, which executes in its own computer process and communicates over its
standard I/O channels in plain text.
...
| Set Input          | `I[GIN]:[VALUE]\n` | `[GIN]` is a non-negative integer which identifies an input interface. `[VALUE]` is a UTF-8 string. The controller interface makes no assumptions about input values and treats them as an opaque strings | Send data from the environment to the controller |
| Set Binary Input   | `B[GIN]:[NUM_BYTES]\n[DATA]` | `[GIN]` is a non-negative integer which identifies an input interface. `[DATA]` is an array of bytes must be read in binary mode | Send an array of bytes from the environment to the controller |
| Get Output         | `O[GIN]\n` | `[GIN]` is a non-negative integer which identifies the output interface | Request for the controller to send an output to the environment |
| Save Controller    | `S[PATH]\n` | `[PATH]` is the filesystem path to save to. If the file already exists then overwrite it. The API will ensure that the parent directory already exists | Save the current state of the controller to file |
| Load Controller    | `L[PATH]\n` | `[PATH]` is the filesystem path to load from | Load the state of the controller from file |
| Custom Message     | `[TYPE][MESSAGE]\n` | `[TYPE]` is a single capital letter, which is not already used by the protocol. `[MESSAGE]` may be any UTF-8 string | Send a custom message to the controller using a new message type |
...
newline character "`\n`". The controller interface makes no assumptions about
the output values and treats them as opaque strings.
...
The environment should forward all such errors to its own management process via
its own stderr channel.  
